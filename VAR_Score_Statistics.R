...
change_model <- ar(var_change, order.max = 1, demean = T, method = "yule-walker")
...
plot(univData)
univ_model <- ar(univData, aic=FALSE, order.max = 1, demean = F, method = "ols")
...
bf_test <- test_Score(x=bf_ts, p=1, G=200, Phi = A_1, eps = eps, alpha = 0.05)
  bf_test
##  change example
change_test <- test_Score(x=var_change, p=1, G=200, Phi = a_change, eps = eps_change, alpha = 0.05) 
change_test$plot
##  nochange example
nochange_test <- test_Score(x=nochange, p=1, G=150, Phi = a_nochange, eps = eps_nochange, alpha = 0.1) 
nochange_test
## univariate example
univ_test <- test_Score(x=matrix(univData), p=1, G= 300, Phi = matrix(a_univ), eps= matrix(eps_univ), alpha=0.1)
univ_test
