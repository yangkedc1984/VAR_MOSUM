...
```{r}
airqual <- read.csv("air-quality-data-2010-2020.csv") #("air-quality-parson-street.csv")
#stpauls <- read.csv("air-quality-stpauls.csv")
#traffic <- read.csv("fact-traffic-counts.csv")
weather <- read.csv("met-data-bristol-2010-2020.csv") #"met-data-bristol-lulsgate.csv")
head(airqual)
#head(stpauls)
#head(traffic)
head(weather)
...
#dfmap <- na.omit(dfmap)
dfmap[,2:6] <- log(dfmap[,2:6]) ##take logs
head(dfmap)
```
daily average
```{r}
...
acf(meteo_models$residuals[,1], lag.max = 28*4, axes=F, main= " "); axis(1,28*(0:4));axis(2) ##monthly
acf(meteo_models$residuals[,1], lag.max = 28*12*4, axes=F, main= " "); axis(1,365*(0:4));axis(2)  ##yearly seasonality
```

```{r}
ardfmap <- ar.ols(meteo_models$residuals, AIC=T, intercept = T) ## this is actually overspecified - try AIC on post-segments
...
```{r}
airqual_mosum_wald <-  test_Wald_new(meteo_models$residuals, p=2, G= 7*52, criterion = "eta", nu = 0.3)
...
```{r}
airqual_mosum_score <- test_Score_new(meteo_models$residuals, p=2, G= 7*52, criterion = "eta", nu = 0.3)
...
acf comparison
```{r}
longest_segment <- meteo_models$residuals[882:1188,1]
acf(longest_segment, main = "")
```
```{r}
airqual_mosum_wald$cps
par(mfrow = c(4,2), mar = 2*c(1, 2, 1, 1))
acf(meteo_models$residuals[1:370,1], main = "", xlab = "", ylab = 1, lag.max = 7)
acf(meteo_models$residuals[371:881,1] , main = "", xlab = "", ylab = 2, lag.max = 7)
acf(meteo_models$residuals[882:1187,1], main = "", xlab = "", ylab = 3, lag.max = 7)
acf(meteo_models$residuals[1188:1513,1], main = "", xlab = "", ylab = 4, lag.max = 7)
acf(meteo_models$residuals[1514:2042,1], main = "", xlab = "", ylab = 5, lag.max = 7)
acf(meteo_models$residuals[2043:2408,1], main = "", xlab = "", ylab = 6, lag.max = 7)
acf(meteo_models$residuals[2409:2703,1], main = "", xlab = "", ylab = 7, lag.max = 7)
acf(meteo_models$residuals[2704:3164,1], main = "", xlab = "", ylab = 8, lag.max = 7)
```
### model diagnostics
```{r}
...
```{r}
#compare sSIC
fit_out_model(meteo_models$residuals, cps=c())$sSIC
fit_out_model(meteo_models$residuals, cps=airqual_mosum_wald$cps)$sSIC
fit_out_model(meteo_models$residuals, cps=airqual_mosum_score$cps)$sSIC
```
